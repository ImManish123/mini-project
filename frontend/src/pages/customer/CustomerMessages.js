...
const CustomerMessages = () => {
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      setMessages(messages.map(m => ({...m, readByReceiver: true})));
      ...
      <div className="page-header" style={{paddingBottom: '60px'}}>
        <div className="container" style={{position: 'relative', zIndex: 1}}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
              <h2 className="fw-bold text-white mb-1" style={{letterSpacing: '-0.5px'}}>My Messages</h2>
        ...
        {messages.length === 0 ? (
          <div className="card border-0 text-center py-5" style={{boxShadow: '0 4px 20px rgba(0,0,0,0.06)', borderRadius: '16px'}}>
            <div className="card-body">
              ...
              <p className="text-muted" style={{fontSize: '14px'}}>You'll receive updates when workers respond to your bookings</p>
            </div>
          </div>
        ) : (
